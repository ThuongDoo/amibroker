...
function negotiateProxies(baseUrl, hubNames, onSuccess, onError, _client) {
  ...
  if (!cleanedHubs || cleanedHubs.length < 1) {
    onError(
      'you must define at least one hub name and they must be typeof string',
    );
    return;
  }
  ...
  if (_client.proxy && _client.proxy.host && _client.proxy.port) {
    negotiateUrlOptions.path =
      negotiateUrlOptions.protocol +
      '//' +
      negotiateUrlOptions.host +
      negotiateUrlOptions.path;
  ...
  if (negotiateUrlOptions.protocol === 'http:') {
    var negotiateResult = http
      .get(negotiateUrlOptions, negotiateFunction)
      .on('error', negotiateErrorFunction);
  } else if (negotiateUrlOptions.protocol === 'wss:') {
    negotiateUrlOptions.protocol = 'https:';
    var negotiateResult = https
      .get(negotiateUrlOptions, negotiateFunction)
      .on('error', negotiateErrorFunction);
...
function getBindings(baseUrl, hubNames, onSuccess, onError, _client) {
  negotiateProxies(
    baseUrl,
    hubNames,
    function (negotiatedOptions) {
      if (!negotiatedOptions.TryWebSockets) {
        onError(
          'This client only supports websockets',
          undefined,
          negotiatedOptions,
        );
        return;
...
function getConnectQueryString(_client) {
  ...
  var connectQueryString =
    _client.url + '/connect?' + querystring.stringify(qs);
...
function clientInterface(baseUrl, hubs, reconnectTimeout, doNotStart) {
  ...
  function scheduleReconnection(isInitalRetry) {
    ...
    if (_client.connection.state == states.connection.retryingConnection) {
      ...
      if (_client.serviceHandlers.reconnecting) {
        ...
        cancelRetry = _client.serviceHandlers.reconnecting.apply(client, [
          retry,
        ]);
      }
      if (!cancelRetry) {
        setTimeout(function () {
          ...
          _client.websocket.client.connect(
            connectQueryString,
            undefined,
            undefined,
            _client.headers,
  ...
  function abort() {
    if (
      ...
      var req = requestObject.request(abortUrlOptions, function (res) {
  ...
  function startCommunication(onSuccess, onError) {
    ...
    if (_client.proxy && _client.proxy.host && _client.proxy.port) {
      startUrlOptions.path =
        startUrlOptions.protocol +
        '//' +
        startUrlOptions.host +
        startUrlOptions.path;
    ...
    if (startUrlOptions.protocol === 'http:') {
      var startResult = http
        .get(startUrlOptions, startFunction)
        .on('error', startErrorFunction);
    } else if (startUrlOptions.protocol === 'wss:') {
      startUrlOptions.protocol = 'https:';
      var startResult = https
        .get(startUrlOptions, startFunction)
        .on('error', startErrorFunction);
  ...
  _client.websocket.client.on('connect', function (connection) {
    ...
    connection.on('message', function (message) {
      ...
      if (_client.serviceHandlers.messageReceived) {
        handled = _client.serviceHandlers.messageReceived.apply(client, [
          message,
        ]);
      }
      if (!handled) {
